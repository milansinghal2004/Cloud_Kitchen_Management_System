      ...
      {!filteredPayments.length ? (
        ...
        <div className="payments-grid-new">
          {filteredPayments.map((order) => {
            ...
            return (
              <article className={`payment-card-premium ${status}`} key={order.id} onClick={() => openDetails(order.id)}>
                <div className="card-top-accent"></div>
                <div className="payment-card-content">
                  <div className="card-header">
                    <span className="order-id">#{order.id.slice(-8)}</span>
                    <div className={`status-icon-pill ${status}`}>
                      {isPaid ? (
                  ...
                  <div className="card-body-main">
                    <div className="amount-display">
                      <span className="currency-label">Total Amount</span>
                      <div className="amount-value">
                        <span className="rs">₹</span>
                        <span className="num">{order.pricing?.total || 0}</span>
                      </div>
                    </div>
                    
                    <div className="payment-date-pill">
                  ...
                  <div className="card-actions-premium">
                    <button className="btn-view" onClick={(e) => { e.stopPropagation(); openDetails(order.id); }}>
                      Details
                    </button>
                    {!isPaid && String(order.status || "").toLowerCase() !== "cancelled" && (
                      <button 
                        className="btn-pay-now" 
                        disabled={isPaying} 
                        onClick={(e) => { e.stopPropagation(); beginPaymentForOrder(order.id); }}
                      >
                        {isPaying ? "..." : "Pay Now"}
                      </button>
                    )}
